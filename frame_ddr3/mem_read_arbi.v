module mem_read_arbi #(
    ...
    input   wire                            rst_n                   ,
    input   wire                            mem_clk                 ,
    input   wire                            ch0_rd_burst_req        ,
    input   wire    [BURST_BITS - 1:0]      ch0_rd_burst_len        ,
    input   wire    [ADDR_BITS - 1:0]       ch0_rd_burst_addr       ,
    output  wire                            ch0_rd_burst_data_valid ,
    output  wire    [MEM_DATA_BITS - 1:0]   ch0_rd_burst_data       ,
    output  wire                            ch0_rd_burst_finish     ,
    
    input   wire                            ch1_rd_burst_req        ,
    input   wire    [BURST_BITS - 1:0]      ch1_rd_burst_len        ,
    input   wire    [ADDR_BITS - 1:0]       ch1_rd_burst_addr       ,
    output  wire                            ch1_rd_burst_data_valid ,
    output  wire    [MEM_DATA_BITS - 1:0]   ch1_rd_burst_data       ,
    output  wire                            ch1_rd_burst_finish     ,
    
    input   wire                            ch2_rd_burst_req        ,
    input   wire    [BURST_BITS - 1:0]      ch2_rd_burst_len        ,
    input   wire    [ADDR_BITS - 1:0]       ch2_rd_burst_addr       ,
    output  wire                            ch2_rd_burst_data_valid ,
    output  wire    [MEM_DATA_BITS - 1:0]   ch2_rd_burst_data       ,
    output  wire                            ch2_rd_burst_finish     ,
    
    input   wire                            ch3_rd_burst_req        ,
    input   wire    [BURST_BITS - 1:0]      ch3_rd_burst_len        ,
    input   wire    [ADDR_BITS - 1:0]       ch3_rd_burst_addr       ,
    output  wire                            ch3_rd_burst_data_valid ,
    output  wire    [MEM_DATA_BITS - 1:0]   ch3_rd_burst_data       ,
    output  wire                            ch3_rd_burst_finish     ,

    input   wire                            ch4_rd_burst_req        ,
    input   wire    [BURST_BITS - 1:0]      ch4_rd_burst_len        ,
    input   wire    [ADDR_BITS - 1:0]       ch4_rd_burst_addr       ,
    output  wire                            ch4_rd_burst_data_valid ,
    output  wire    [MEM_DATA_BITS - 1:0]   ch4_rd_burst_data       ,
    output  wire                            ch4_rd_burst_finish     ,

    input   wire                            ch5_rd_burst_req        ,
    input   wire    [BURST_BITS - 1:0]      ch5_rd_burst_len        ,
    input   wire    [ADDR_BITS - 1:0]       ch5_rd_burst_addr       ,
    output  wire                            ch5_rd_burst_data_valid ,
    output  wire    [MEM_DATA_BITS - 1:0]   ch5_rd_burst_data       ,
    output  wire                            ch5_rd_burst_finish     ,

    output  reg                             rd_burst_req            ,
    output  reg     [BURST_BITS - 1:0]      rd_burst_len            ,
    output  reg     [ADDR_BITS - 1:0]       rd_burst_addr           ,
    input   wire                            rd_burst_data_valid     ,
    input   wire    [MEM_DATA_BITS - 1:0]   rd_burst_data           ,
...
localparam CH4_CHECK = 6'd17;
localparam CH4_BEGIN = 6'd18;
localparam CH4_READ = 6'd19;
localparam CH4_END = 6'd20;

localparam CH5_CHECK = 6'd21;
localparam CH5_BEGIN = 6'd22;
localparam CH5_READ = 6'd23;
localparam CH5_END = 6'd24;
...
begin
    case(read_state)
        ...
        //
        CH0_CHECK:
            if(ch0_rd_burst_req && ch0_rd_burst_len != {BURST_BITS{1'd0}})
        ...
        //	
        CH1_CHECK:
            if(ch1_rd_burst_req && ch1_rd_burst_len != {BURST_BITS{1'd0}})
        ...
        //	
        CH2_CHECK:
            if(ch2_rd_burst_req  && ch2_rd_burst_len != {BURST_BITS{1'd0}})
        ...
        //	
        CH3_CHECK:
            if(ch3_rd_burst_req  && ch3_rd_burst_len != {BURST_BITS{1'd0}})
        ...
        //	
        CH4_CHECK:
            if(ch4_rd_burst_req  && ch4_rd_burst_len != {BURST_BITS{1'd0}})
        ...
        //	
        CH5_CHECK:
            if(ch5_rd_burst_req  && ch5_rd_burst_len != {BURST_BITS{1'd0}})
...
begin
    if(~rst_n)
        rd_burst_req <= 1'b0;
    else if(read_state == CH0_BEGIN || read_state == CH1_BEGIN || read_state == CH2_BEGIN || read_state == CH3_BEGIN || read_state == CH4_BEGIN || read_state == CH5_BEGIN)
        rd_burst_req <= 1'b1;
    else if(rd_burst_data_valid || read_state == CH0_CHECK  || read_state == CH1_CHECK  || read_state == CH2_CHECK  || read_state == CH3_CHECK || read_state == CH4_CHECK || read_state == CH5_CHECK)
        rd_burst_req <= 1'b0;
    else
