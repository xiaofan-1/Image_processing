...
(
    ...
    input          rst_n        , // reset signal,active low                 复位信号，低电平有效
...
always @ (posedge clk or negedge rst_n) begin
    ...
    begin
        ...
        begin
            case(tx_state)
                IDLE       :  uart_tx  <= 1'h1;           //空闲状态输出高电平
                SEND_START :  uart_tx  <= 1'h0;           //start状态发送一个波特周期的低电平
                ...
                SEND_STOP  :  uart_tx  <= 1'h1;          //发送停止状态 输出1个波特周期高电平
                default    :  uart_tx  <= 1'h1;          // 其他状态默认与空闲状态一致，保持高电平输出
            endcase
        end
        else
            uart_tx <= 1'h1;
