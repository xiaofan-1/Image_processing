...
//debug PIC1   帧差之后的结果
	output              PIC1_frame_vsync	,
    output       		PIC1_frame_href  	,
	output				PIC1_frame_clken	,
	output				PIC1_img_Bit		,
	
//debug PIC2	腐蚀膨胀之后的结果
	output            	PIC2_frame_vsync	,
    output      	  	PIC2_frame_href  	,
	output				PIC2_frame_clken	,
	output				PIC2_img_Bit		,  
	
//user interface
	input		[7:0]	user_Threshold	,	//  帧差阈值
	input		[9:0]	MIN_DIST		,	//	多目标之间的最小间距
    output      [19:0]  seg_data      	
);
...
//--------------------------------------
//Bit Image Process with add rectangularC after onnected Component. 
...
/* 
	//在“二值化&腐蚀&膨胀”之后的结果上画方框
	.per_frame_vsync		(post3_frame_vsync),	 
	.per_frame_href			(post3_frame_href),		 
	.per_frame_clken		(post3_frame_clken),	 
	.per_img_red			({8{post3_img_Bit}}), 
	.per_img_green			({8{post3_img_Bit}}), 
	.per_img_blue			({8{post3_img_Bit}}), 
*/
...
// 输出结果 帧差之后的结果
assign	PIC1_frame_vsync	=	post1_frame_vsync	;
assign	PIC1_frame_href		=	post1_frame_href	;
assign	PIC1_frame_clken	=	post1_frame_clken	;
assign	PIC1_img_Bit		=	post1_img_Bit		;

// 输出结果 腐蚀&膨胀之后的结果
assign	PIC2_frame_vsync	=	post3_frame_vsync 	;
assign	PIC2_frame_href 	=	post3_frame_href 	;
assign	PIC2_frame_clken	=	post3_frame_clken 	;
assign	PIC2_img_Bit		=	post3_img_Bit 		;
